...
/**
 * Fetch from Abuse.ch URLhaus — recent malware URLs
 */
...
/**
 * Fetch from MITRE ATT&CK — enterprise techniques
 */
async function fetchMITREAttack(): Promise<ThreatIntel[]> {
  try {
    ...
    // Get attack patterns (techniques) — focus on the most recent revocations/additions
...
/**
 * Fetch from FIRST EPSS — Exploit Prediction Scoring System
 */
  ...
  try {
    ...
    return entries.slice(0, 50).map((entry: any) => {
      ...
      return {
        ...
        description: `Exploit Prediction Score: ${(epssScore * 100).toFixed(2)}% | Percentile: ${(percentile * 100).toFixed(1)}% — This CVE is in the top ${((1 - percentile) * 100).toFixed(1)}% most likely to be exploited in the next 30 days.`,
        cveIds: [entry.cve],
        affectedProducts: [],
...
/**
 * Fetch from AlienVault OTX — Open Threat Exchange pulses
 */
...
// ============================================
// SHARED DATA BRIDGE — Evolution ↔ Intelligence
// ============================================
...
async function runEvolutionCycle(aggressive: boolean = false): Promise<{
  ...
  console.log(`🧠 Evolution cycle starting (${aggressive ? 'AGGRESSIVE' : 'normal'} mode)...`);
  ...
  for (const feed of settledFeeds) {
    if (feed.result.status === 'fulfilled' && feed.result.value.length > 0) {
      ...
      console.log(`  ⚠️ ${feed.name}: 0 items`);
    }
  }
  ...
  console.log(`🧠 Cycle complete: ${results.newThreats} threats, ${results.newRules} rules from ${results.sources.length} sources`);
...
export async function aiEvolutionRoutes(app: FastifyInstance) {
  ...
  app.post('/ai-evolution/bootstrap', { preHandler: authMiddleware() }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    console.log('🚀 BOOTSTRAP MODE: Seeding evolution engine with ALL available data...');
    ...
    console.log('⚡ Switching to 15-minute evolution cycles for 24 hours...');
    evolutionInterval = setInterval(async () => {
      console.log('🧠 Running accelerated evolution cycle...');
      const results = await runEvolutionCycle(false);
      console.log(`🧠 Accelerated cycle: ${results.newThreats} threats, ${results.newRules} rules`);
    ...
    setTimeout(() => {
      if (evolutionInterval) {
        ...
        evolutionInterval = setInterval(async () => {
          ...
          const results = await runEvolutionCycle(false);
          console.log(`🧠 Evolution complete: ${results.newThreats} threats, ${results.newRules} rules`);
        }, 60 * 60 * 1000);
        console.log('🧠 Reverted to hourly evolution cycles');
      }
    }, 24 * 60 * 60 * 1000);
    ...
    return {
      success: true,
      message: '🚀 BOOTSTRAP COMPLETE — Evolution engine is now fully seeded and running in accelerated mode',

import React from 'react';

const PenetrationTesting: React.FC = () => {
  const tests = [
    { id: 'p-1', name: 'External App Test', status: 'Running', findings: 5, severity: 'High' },
    { id: 'p-2', name: 'Internal Network', status: 'Scheduled', findings: 0, severity: 'N/A' },
    { id: 'p-3', name: 'API Security', status: 'Completed', findings: 9, severity: 'Critical' },
  ];

  const stats = [
    { label: 'Engagements (Q1)', value: 14 },
    { label: 'Exploits validated', value: 22 },
    { label: 'Retests completed', value: 9 },
    { label: 'Mean time to fix', value: '9d' },
  ];

  return (
    <div className="min-h-screen bg-slate-900 text-white p-6 space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-amber-400">Penetration Testing</h1>
          <p className="text-slate-400">Adversarial testing, exploit validation, and remediation verification.</p>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm text-slate-300">Scoped & approved</div>
      </header>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map(card => (
          <div key={card.label} className="bg-slate-800 border border-slate-700 rounded-xl p-4">
            <div className="text-slate-400 text-sm">{card.label}</div>
            <div className="text-2xl font-semibold mt-1">{card.value}</div>
          </div>
        ))}
      </div>

      <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold">Engagements</h2>
          <span className="text-xs text-slate-400">Pipeline</span>
        </div>
        <div className="space-y-3">
          {tests.map(item => (
            <div key={item.id} className="bg-slate-900 border border-slate-700 rounded-lg p-3 text-sm flex items-center justify-between">
              <div>
                <div className="font-semibold">{item.name}</div>
                <div className="text-xs text-slate-500">Findings: {item.findings}</div>
              </div>
              <div className="text-right text-xs text-green-300">
                <div>{item.status}</div>
                <div className="text-slate-400">Severity: {item.severity}</div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default PenetrationTesting;

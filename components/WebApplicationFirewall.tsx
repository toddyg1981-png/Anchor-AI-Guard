...
const WebApplicationFirewall: React.FC = () => {
  ...
  const { loading, analyzing, analysisResult, runAnalysis, clearAnalysis } = useSecurityModule('waf', {
    liveTraffic, wafRules, blockedRequests, rateLimits, analytics,
  });
  ...
  if (loading) return (<div className="bg-slate-900 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-red-400" /></div>);
    ...
    <div className="bg-slate-900 text-white p-6 space-y-6">
      ...
      {analysisResult && (
        <div className="bg-slate-800 border border-red-700 rounded-xl p-4 space-y-2">
          <div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-red-400">🤖 AI Analysis</h2><button onClick={() => clearAnalysis()} className="text-slate-400 hover:text-white text-sm">✕</button></div>
